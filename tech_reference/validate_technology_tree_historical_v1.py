...
P=pathlib.Path(__file__).resolve().parent/"technology_tree_historical_v1.csv"
with P.open(encoding="utf-8-sig",newline="") as f: rows=list(csv.DictReader(f))
nodes={r["TECH"]:r for r in rows}
succ=defaultdict(list);indeg={n:0 for n in nodes};err=[]
for n,r in nodes.items():
    ps=[x.strip() for x in r["PREREQUISITES_AND"].split(";") if x.strip()]
    if len(ps)>3:err.append(f"{n}: >3 prerequisites")
    for p in ps:
